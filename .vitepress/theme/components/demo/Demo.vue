...
function getLayout(id: number) {
	...
	return name === 'cover' ? Cover : name === 'center' ? Center : Default;
}
...
onMounted(() => {
	new TypeIt(block.value, {
		...
		.type(
			'<br><span class="token title"># Bem-vindo ao Slidev!</span><br><br>',
			{ delay: 400 }
		)
		.type('Apresentações para desenvolvedores', { delay: 400 })
		.move('START', { speed: 0 })
		.type('<br>')
		.move('START')
		.exec(pause)
		...
		.move('END', { speed: 0 })
		.exec(pause)
		.type('<br><br><span class="token punctuation">---</span><br><br>', {
			delay: 400,
		})
		.exec(resume)
		.exec(() => setTimeout(() => (page.value = 1)))
		.type('<span class="token title"># Página 2</span><br><br>', {
			delay: 400,
		})
...
<template>
	<div>
		<DemoEditor>
			<div class="text-sm opacity-50">./slides.md</div>
